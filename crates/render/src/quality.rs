...
impl RenderQuality {
    ...
    /// Longest source edge worth decoding for this quality.
    #[must_use]
    pub const fn source_edge(self, aspect: Aspect) -> u32 {
        let (w, h) = self.frame_size(aspect);
        if w > h { w } else { h }
    }
}
...
mod tests {
    ...
    fn full_delegates_to_resolution() {
        ...
        assert_eq!(
            RenderQuality::Full(Resolution::Uhd4k).source_edge(Aspect::Portrait9x16),
            3840
        );
