...
| Music "Lower under video sound", "Loop" (`editor.slint`, M5) | Switch | on/off, applied at once | Fluent: toggle switch for settings that take effect immediately; std Switch labels cannot wrap, so the label is short and the detail ("Repeats the songs until the slideshow ends.") is a caption below | ✅ | `music.png` |
