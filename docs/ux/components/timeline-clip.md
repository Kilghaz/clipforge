...
| Colour card (M5) | decision note `docs/ux/decisions/m5b-text-track.md` | background colour (`title-bg-*`, same values as the renderer) instead of a thumbnail, `title-clip` stripe, footer "Colour card"; its title text lives on the text track | ✅ | gallery "Timeline clips, M5" |
...
| Scrolling | NN/G direct manipulation: drags act on what is under the pointer | the strip scrolls with the wheel / trackpad only (a vertical wheel scrolls sideways); drag panning is off so drags always move, trim or scrub | ✅ | `dragging_a_clip_in_a_long_timeline_moves_the_clip_not_the_view`, `the_wheel_scrolls_a_long_timeline` |
